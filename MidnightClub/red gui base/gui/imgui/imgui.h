...
namespace ImGui
{
    ...
    IMGUI_API bool          cCheckbox(const char* label, bool* v, ImColor C);
    ...
    //   If you get a warning converting a float to ImGuiSliderFlags, read https://github.com/ocornut/imgui/issues/3361
    IMGUI_API bool          cSliderFloat(const char* label, float* v, float v_min, float v_max, ImColor C , const char* format = "%.3f", ImGuiSliderFlags flags = 0);
    IMGUI_API bool          cSliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, ImColor C , const void* p_max, const char* format = NULL, ImGuiSliderFlags flags = 0);
    IMGUI_API bool          SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format = "%.3f", ImGuiSliderFlags flags = 0);     // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.  // adjust format to decorate the value with a prefix or a suffix for in-slider labels or unit display.
...
struct ImGuiStyle
{
    ...
    ImVec2      FramePadding;  
    ImVec2      TestFramePadding;           // Padding within a framed rectangle (used by most widgets).
